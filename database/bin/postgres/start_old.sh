...
function initialize_primary() {
    ...
    if [ ! -f ${PGDATA?}/postgresql.conf ]; then
        ...
        # load ecrs db
        psql -U postgres -p "${PG_PRIMARY_PORT}" < /tmp/createecrsusers.sql
        psql -U postgres -p "${PG_PRIMARY_PORT}" -c "create database ecrsdbdemo"
        psql -U postgres -p "${PG_PRIMARY_PORT}" -d ecrsdbdemo --single-transaction -v ON_ERROR_STOP=1 -f /tmp/ecrsdb.sql
        
        psql -U postgres -p "${PG_PRIMARY_PORT}" -d ecrsdbdemo < /tmp/assign_privs.sql
        psql -U postgres -p "${PG_PRIMARY_PORT}" -d ecrsdbdemo < /tmp/define_demodbusers.sql
