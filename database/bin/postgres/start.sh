...
/usr/bin/python /opt/calc_server/calc_server.py &
...
#echo "call mkdir pgdata in other place"
...
function initdb_logic() {
    ...
    cmd="/usr/pgsql-11/bin/initdb -D $PGDATA "
    if [[ -v PG_LOCALE ]]; then
...
function initialize_primary() {
    ...
    echo "pgdata environment variable ->"$PGDATA"<-"
    ...
    if [ ! -f ${PGDATA?}/postgresql.conf ]; then
        ...
        echo "call mkdir pgdata in initialize_primary"
        mkdir -p ${PGDATA?}
        ...
        #echo_info "Temporarily starting database to run chemrps setup.."
        #su postgres
        #pwd

        cmd="/usr/pgsql-11/bin/pg_ctl -D ${PGDATA?}"
        cmd+=" -l /tmp/mypostgresqllogfile start"
        #cmd+=" -o -c listen_addresses='' ${PG_CTL_OPTS:-} start"
        ...
        while true; do
            ...
            if [ $? -eq 0 ]; then
                ...
                #echo_info "The database is ready for chemrps setup"
                break
            fi
            sleep 2
        done
        ...
        #echo_info "Loading chemrps setup.." >> /tmp/start-db.log
        ...
        sed -i "s/DBSUPER_USER/$SUPER_USER/g" /tmp/createchemrpsusers.sql
        sed -i "s/SUPERUSERPASSWORD/$SUPERUSERPASSWORD/g" /tmp/createchemrpsusers.sql
        sed -i "s/BCF_AUTH_PASSWORD/$BCF_AUTH_PASSWORD/g" /tmp/createchemrpsusers.sql
        
        
        sed -i "s/BCF_AUTHENTICATORPASSWORD/$BCF_AUTHENTICATORPASSWORD/g" /tmp/createchemrpsusers.sql
        sed -i "s/BCF_REGPASSWORD/$BCF_REGPASSWORD/g" /tmp/createchemrpsusers.sql
        sed -i "s/BCF_REG_FACADEPASSWORD/$BCF_REG_FACADEPASSWORD/g" /tmp/createchemrpsusers.sql
        
        echo_info "Loading chemrps setup.."
        

        psql -U postgres -p "${PG_PRIMARY_PORT}" < /tmp/createchemrpsusers.sql
        
        
        
        #psql -U postgres -p "${PG_PRIMARY_PORT}" -c "create database chemrpsdbdemo"
        psql -U postgres -p "${PG_PRIMARY_PORT}" -c "create database $DATABASENAME"
        
        #psql -U postgres -p "${PG_PRIMARY_PORT}" -d chemrpsdbdemo --single-transaction -v ON_ERROR_STOP=1 -f /tmp/chemrpsdb.sql
        
        psql -U postgres -p "${PG_PRIMARY_PORT}" -d $DATABASENAME  --single-transaction -v ON_ERROR_STOP=1 -f /tmp/chemrpsdb.sql
        
        # psql -U postgres -p "${PG_PRIMARY_PORT}" -d chemrpsdbdemo < /tmp/assign_privs.sql
        
               
        #psql -U postgres -p "${PG_PRIMARY_PORT}" -d chemrpsdbdemo < /tmp/define_demodbusers.sql
        
        psql -U postgres -p "${PG_PRIMARY_PORT}" -d $DATABASENAME < /tmp/define_demodbusers.sql
        
        # psql -U postgres -p "${PG_PRIMARY_PORT}" -d ecrsdbdemo < /tmp/2019-09-21_SimilaritySearch.sql

        #psql -U postgres -p "${PG_PRIMARY_PORT}" -d chemrpsdbdemo < /tmp/2019-11-21_SimilaritySearch_enhancements.sql
        
        psql -U postgres -p "${PG_PRIMARY_PORT}" -d $DATABASENAME < /tmp/2019-11-21_SimilaritySearch_enhancements.sql
        
        
        
        echo_info "Stopping database after primary initialization.."
        #pg_ctl -D $PGDATA --mode=fast stop
        cmd="/usr/pgsql-11/bin/pg_ctl -D $PGDATA --mode=fast stop"
        su postgres -c "$cmd"
        if [[ -v SYNC_REPLICA ]]; then
...
cmd="/usr/pgsql-11/bin/postgres -D $PGDATA"
su postgres -c "$cmd" &
